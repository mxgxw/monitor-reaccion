...
class XBee900HP:
  ...
  def start(self):
    ...
    self.init()
    if self.debug: print "Modem in API Mode 2. Listening for messages."
  ...
  def init(self):
    while self.waitForInit:
      if self.xBeeStatus == ModemStatus.WAIT_MODEM:
        if self.debug: print 'Entering command mode.'
        self.HardSerial.write("+++");
        if self.waitFor("OK"):
          if self.debug: print 'Now in command mode, changing to API MODE 2'
        ...
        if confirmATAP2 and confirmATCN:
          ...
          if self.debug: print "Modem detected, switched to API Mode 2."
        else:
          self.xBeeStatus = ModemStatus.WAIT_MODEM
          if self.debug: print "Modem not detected, keep waiting for modem."
  ...
  def listen(self):
    if self.HardSerial.inWaiting()>0 and self.xBeeStatus == ModemStatus.API_MODE2:
      ...
      if self.debug: print "Reading data"
      ...
      if self.readStatus == ReadStatus.RSP_WAIT:
        ...
        if self.debug: print "RSP_RDDATA"
        if self.rcvSize<self.packetSize:
          ...
          if self.checkSum == 0xFF:
            if self.frameReceivedHandler != None:
              if self.debug: print "Frame lenght: "+str(len(self.rcvBuffer))
              self.frameReceivedHandler(self.rcvBuffer)
          if self.debug: print "End of Frame"
          self.readStatus = ReadStatus.RSP_WAIT
  ...
  def waitFor(self, response):
    ...
    while (curr_time-millis())<10000:
      if self.HardSerial.inWaiting()>0:
        ...
        if self.debug: print self.buff
        if '\r' in self.buff:
          if self.debug: print 'Endline received'
